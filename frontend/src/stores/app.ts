...
function normalizePublicSiteName(value?: string | null): string {
  ...
  return /^oneapi[-\s_]*th$/i.test(trimmed) ? DEFAULT_SITE_NAME : trimmed
}
...
export const useAppStore = defineStore('app', () => {
  ...
  const contactInfo = ref<string>(DEFAULT_CONTACT_EMAIL)
  ...
  function applySettings(config: PublicSettings): void {
    ...
    contactInfo.value = normalizedConfig.contact_info || DEFAULT_CONTACT_EMAIL
  ...
  async function fetchPublicSettings(force = false): Promise<PublicSettings | null> {
    ...
    if (publicSettingsLoaded.value && !force) {
      ...
      return {
        ...
        contact_info: contactInfo.value || DEFAULT_CONTACT_EMAIL,
