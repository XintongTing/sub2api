  ...
  admin: {
    ...
    settings: {
      ...
      payment: {
        ...
        field_payoneerApiBaseHint: '沙箱默认 https://api.sandbox.payoneer.com，生产默认 https://api.payoneer.com。仅在 Payoneer 为商户提供不同端点时覆盖。',
        field_payoneerPathHint: 'Payoneer Checkout 高级适配路径。查询路径需保留 {trade_no}，用于替换为上游交易号后做订单核验。',
        ...
        payoneerWebhookHint: '后端 Payoneer Provider 启用后，请在 Payoneer Checkout 后台将以下地址配置为 Webhook 端点。',
